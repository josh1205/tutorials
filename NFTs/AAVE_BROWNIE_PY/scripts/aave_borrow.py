...
def main():
    ...
    borrowable_eth, total_debt = get_borrowed_data(lending_pool, account)
    print("Lets borrow")
    # DAI in terms of ETH

    dai_eth_price = get_asset_price(config["networks"][network.show_active()]["dai_eth_price_feed"]) #only mainnet-fork
    amount_dai_to_borrow = (1/dai_eth_price) * (borrowable_eth * 0.95)
    # borrowable -> borrowable_dai * 95%
    ...
    get_borrowed_data(lending_pool, account)
...
def get_borrowed_data(lending_pool, account):
    (
...
def get_lending_pool():
    #When workin with a contract we need an ABI and an address
